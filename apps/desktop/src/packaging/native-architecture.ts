...
export function assertNativeArchitecture(directory: string, architecture: string): void {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (/\.(?:node|exe|dll)$/i.test(entry.name)) assertNativeFileArchitecture(path, architecture);
  }
}

function expectedMachine(architecture: string): number {
...
export function assertNativeFileArchitecture(path: string, architecture: string): void {
  const expected = expectedMachine(architecture);
