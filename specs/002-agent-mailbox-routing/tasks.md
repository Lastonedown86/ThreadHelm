...
- [ ] T122 [US7] Add a final launch-settings disclosure handoff proving generated provider/model/effort/isolation/budget/tool/workspace values remain separately resolved in `packages/providers/src/adapter.ts` and `apps/desktop/src/main/coordination/profile-wizard.ts`
...
- [ ] T126 [US8] Verify OpenAI `gpt-5.6-sol` at `max`, fallback `gpt-5.6-terra` at `max`, Claude `claude-opus-5` at `xhigh`, human owner, and Antigravity `gemini-3.1-pro-high` adversarial reviewer availability; record the complete authority/concurrency gate in `specs/002-agent-mailbox-routing/execution-evidence.md`
