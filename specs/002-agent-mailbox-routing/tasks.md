...
**Goal**: Import portable Marvel-themed agent manifests through an exact, digest-bound review while
...
- [X] T095 [P] [US6] Add sanitized valid, duplicate, revised, hostile-text, unavailable-model, excessive-bound, and changed-after-preview agent manifest fixtures in `packages/test-fixtures/src/agent-profiles.ts` and export them from `packages/test-fixtures/src/index.ts`
